...
const UserSportsEquipment = () => {
    ...
    const userEmail = user?.email; // Use the logged-in user's email here (can be dynamic)



    useEffect(() => {
        fetch(`http://localhost:5000/sports/email/${userEmail}`)
            .then((res) => res.json())
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {sports.map((sport) => (
                    <div key={sport._id} className="bg-white p-4 rounded-lg shadow-lg">
                        <img
                            src={sport.image}
                            alt={sport.name}
